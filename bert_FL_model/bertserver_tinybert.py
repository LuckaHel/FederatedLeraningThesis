    ...
    def evaluate(server_round, parameters, config):
        ...
        with torch.no_grad():
            for input_ids, attention_mask, targets in test_loader:
                ...
                loss += criterion(torch.sigmoid(outputs), targets).item()
...
model = AutoModelForSequenceClassification.from_pretrained("huawei-noah/TinyBERT_General_4L_312D", num_labels=16)
...
fl.server.start_server(server_address="localhost:9090", strategy=strategy, config=fl.server.ServerConfig(num_rounds=3))
